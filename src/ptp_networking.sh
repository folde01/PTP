...
# tap0 needs an IP on 10.0.2.0/24 (emulator net) for the host:
...
# and B) its destination is an IP address that, by the host's routing tables (hence use of the POSTROUTING chain), should be routed via interface wlp3s0, then: 
# change the packet's source IP to be the IP of wlp3s0 (the host's outgoing interface) and keep track of this IP address translation
